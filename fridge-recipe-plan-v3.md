...
> - **v3.12: Ablation 4종 완료·본 학습 정합성** — `ablation_results_4.json`, §6.2.5 종합 표, `train_yolo_byclaude.py` 대조표, §5.1.2 Transfer Learning 수정
...
- **애플리케이션**: **FastAPI** 추론 API + **Streamlit** UI + **Docker** 재현 가능 환경
- **데이터 활용 원칙**:
...
| ✅ FastAPI + Streamlit + Docker 배포 | ❌ Cold-start / Session-based RecSys |
...
**`smart refrigerator.yolov11` (Roboflow export + 수동 정리, 2026.06)**:
...
| `smart refrigerator_before.yolov11` | leakage 있음 | **사용 금지** (아카이브만) |
...
**학습 설정 (Kaggle, `train_yolo_byclaude.py`)**: YOLOv11n, COCO pretrained, AdamW lr=0.001, CosineAnnealingLR, 100 epoch (EarlyStop @62, best @42), batch=16, imgsz=640, 가중치 `best.pt`.
...
!python ablation_yolo.py --exp 1   # 아키텍처
!python ablation_yolo.py --exp 2   # Augmentation
!python ablation_yolo.py --exp 3   # Transfer Learning
!python ablation_yolo.py --exp 4   # Optimizer
```
산출물: `ablation_results_1.json` ~ `_4.json` → `presentation/ablation/`에 저장 권장

### 실험 1: YOLO 아키텍처 비교 *(2026.06.19 완료 — val 150장, 50 epoch, AdamW 공통)*
...
**출처**: Kaggle `ablation_yolo.py --exp 1` → `presentation/ablation/ablation_results_1.json`

### 실험 2: Data Augmentation 전략 비교 *(2026.06.19 완료 — YOLOv11n 고정, val 150장, 50 epoch)*
...
**최종 채택 증강 (본 학습 `train_yolo_byclaude.py`와 동일)**
...
**출처**: Kaggle `ablation_yolo.py --exp 2` → `presentation/ablation/ablation_results_2.json`

### 실험 3: Transfer Learning 효과 검증 *(2026.06.19 완료 — YOLOv11n 고정, val 150장, 50 epoch, 실험 2 증강 동일)*
...
**최종 채택: COCO pretrained + full fine-tune (본 학습 `train_yolo_byclaude.py`와 동일)**
...
**출처**: Kaggle `ablation_yolo.py --exp 3` → `presentation/ablation/ablation_results_3.json`

### 실험 4: Optimizer · Learning Rate 비교 *(2026.06.19 완료 — YOLOv11n 고정, val 150장, 50 epoch, 실험 2·3 증강·pretrain 동일)*
...
**최종 채택 Optimizer (본 학습 `train_yolo_byclaude.py`와 동일)**
...
**출처**: Kaggle `ablation_yolo.py --exp 4` → `presentation/ablation/ablation_results_4.json`
...
| 구분 | Ablation (`ablation_yolo.py`) | 본 학습 (`train_yolo_byclaude.py` → `best.pt`) |
|---|---|---|
...
### 6.2.6 `train_yolo_byclaude.py` ↔ Ablation 채택 설정 정합성

> Ablation 4종에서 확정한 **채택 조합**이 본 학습 스크립트와 **코드 파라미터 수준에서 일치**하는지 대조한다.

| 항목 | `train_yolo_byclaude.py` | Ablation 채택 (exp1n·2·3·4) | 일치 |
|---|---|---|:-:|
...
1. **모델·전이학습·증강·Optimizer 4축** — Ablation 채택값과 `train_yolo_byclaude.py` **100% 일치** (§6.2.5 수렴점 0.826/0.548으로 교차 검증).
...
**발표용 한 줄**: “4종 Ablation이 각각 YOLOv11n · Mosaic+HSV · COCO full fine-tune · AdamW+Cosine을 독립 검증했고, 네 실험 모두 val 0.826으로 수렴 — 동일 설정의 `train_yolo_byclaude.py` 본 학습에서 test **0.870** 달성.”

## 6.4 프로젝트 진행 현황 및 산출물 *(2026.06.19 기준)*
...
| YOLO 학습 | ✅ | `best.pt` (test mAP 0.87), `train_yolo_byclaude.py` |
...
| **Ablation 4종 종합** | ✅ | §6.2.5~6.2.6 (`train_yolo_byclaude.py` 정합 대조) |

### 미완 / 다음 작업

| 순서 | 작업 | 산출물 |
|:-:|---|---|
| 1 | 로컬 E2E 시연 캡처 | `presentation/screenshots/` (Streamlit·Swagger) |
| 2 | Docker | `Dockerfile`, `docker-compose.yml` |
| 3 | PPT·시연 영상 | 발표 슬라이드, 2~3분 데모 |
| 4 | README 최종 | 실행·재현 가이드 — **v3.16 반영 완료** |

### 로컬 실행 (시연·캡처용)

```bash
cd 프로젝트 && source .venv/bin/activate
...
| 항목 | 목표 |
|---|---|
| End-to-End 응답 시간 | 이미지 업로드 → 추천 ≤ 5초 |
| Docker 이미지 크기 | ≤ 3GB |
| Docker Compose 기동 | 1회 명령 실행 |
| API 문서화 | FastAPI Swagger 자동 제공 |
| 재현성 | 가중치 + requirements + docker-compose.yml |
...
| Week 4 말 | ✅ | Ablation **4/4 완료** — §6.2.5~6.2.6 종합·`train_yolo_byclaude.py` 정합 |
| Week 6 말 | ✅ | End-to-End 통합 (FastAPI + Streamlit UI) |
| Week 7 | ⬜ | Docker, E2E 벤치마크 |
...
| **5. 모델 및 로직** | YOLO + Ranker + Pantry + Scaler | `train_yolo_byclaude.py` 설정, §5.3~5.5, `core/` 모듈 구조 |
...
| `smart refrigerator_before.yolov11/` | leakage 데이터 |
...
| 본 학습 정합 | 언급만 | **§6.2.6 `train_yolo_byclaude.py` 대조표** |
