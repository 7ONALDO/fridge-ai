...
KOREAN_UNITS = (
    r"g|kg|mg|ml|l|L|개|마리|줄기|쪽|큰술|작은술|컵|모|장|봉|팩|포기|송이|꼬지|인분|온스|파운드|테이블스푼|티스푼"
)
...
# 식약처 API: 돼지고기(70g) — 이름에 숫자·괄호 없음
KO_ITEM_NAME_PARENS = re.compile(
    rf"^([^(\d]+?)\s*\((\d+\.?\d*)\s*({KOREAN_UNITS})\)\s*$",
    re.IGNORECASE,
)
...
def _strip_section_prefix(text: str) -> str:
    return SECTION_HEADER.sub("", text.strip()).strip()
...
def _parse_korean_item(raw: str) -> ParsedIngredient | None:
    ...
    # 조리 설명 문자열 필터 ("껍질을 벗기고 씨를 제거한" 등)
    if not re.search(r"\d", raw) and _PREP_NOTE.search(raw):
        return None
    ...
    name = re.sub(r"\d+[gml개마리].*", "", name).strip()
    if len(name) < 2:
        return None
    ...
    *,
    ...
    if structured is not None:
        ...
        for line in text.splitlines():
            ...
            if not cleaned:
                continue
            for part in _split_outside_parens(cleaned, ","):
